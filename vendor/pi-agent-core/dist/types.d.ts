...
/**
 * A one-off loop phase (compress, critic, …) streamed with a throwaway context.
 * The assistant reply is emitted for TUI/session visibility, but is not merged
 * into the actor transcript used by the inner loop.
 */
export interface SpecialTurnRequest {
    /** Short label for logs/TUI, e.g. "compress" or "critic". */
    label: string;
    /** Throwaway LLM context. `tools` should be empty so this phase cannot call tools. */
    context: AgentContext;
    convertToLlm?: (messages: AgentMessage[]) => Message[] | Promise<Message[]>;
    maxTokens?: number;
    onComplete?: (message: AssistantMessage) => void | Promise<void>;
}
export interface AgentLoopConfig extends SimpleStreamOptions {
    ...
    /**
     * Called after `prepareNextTurn` and before `shouldStopAfterTurn`.
     * Return a special turn to stream an isolated provider request (empty tools,
     * no actor transcript). Repeat until this returns undefined.
     *
     * Contract: must not throw or reject. Return undefined when no special phase is needed.
     */
    getSpecialTurn?: (context: PrepareNextTurnContext) => Promise<SpecialTurnRequest | undefined>;
    /**
