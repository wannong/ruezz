...
async function runLoop(initialContext, newMessages, initialConfig, signal, emit, streamFn) {
    ...
    while (true) {
        ...
        while (hasMoreToolCalls || pendingMessages.length > 0) {
            ...
            while (config.getSpecialTurn) {
                const special = await config.getSpecialTurn({
                    ...nextTurnContext,
                    context: currentContext,
                });
                if (!special) {
                ...
                ranSpecial = true;
                await runSpecialTurn(special, currentContext, newMessages, config, signal, emit, streamFn);
            }
            if (ranSpecial) {
...
/**
 * Stream a compress/critic (or similar) phase on a throwaway context.
 * Does not merge that context into the actor transcript, and does not execute tools.
 */
async function runSpecialTurn(special, _actorContext, newMessages, config, signal, emit, streamFn) {
    console.error(`[my-pi] phase=${special.label}`);
    await emit({ type: "turn_start" });
    for (const prompt of special.context.messages) {
        await emit({ type: "message_start", message: prompt });
        await emit({ type: "message_end", message: prompt });
        newMessages.push(prompt);
    }
    const throwaway = {
        systemPrompt: special.context.systemPrompt,
        messages: [...special.context.messages],
        tools: [],
    };
    const specialConfig = {
        ...config,
        transformContext: undefined,
        convertToLlm: special.convertToLlm ?? config.convertToLlm,
        maxTokens: special.maxTokens ?? config.maxTokens,
    };
    const message = await streamAssistantResponse(throwaway, specialConfig, signal, emit, streamFn);
    newMessages.push(message);
    await emit({ type: "turn_end", message, toolResults: [] });
    try {
        await special.onComplete?.(message);
    }
    catch (error) {
        const text = error instanceof Error ? error.message : String(error);
        console.error(`[my-pi] phase=${special.label} onComplete failed: ${text}`);
    }
}
/**
