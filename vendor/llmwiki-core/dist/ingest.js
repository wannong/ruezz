...
export function buildAnalysisPrompt(source, ctx) {
    return [
        ...
        `Do NOT propose splitting a complete article into one page per heading.`,
        ``,
...
export function buildGenerationPrompt(source, analysis, ctx) {
    return [
        ...
        `- Prefer one wiki/sources page that keeps the source whole. Do not mint a new concept page per heading.`,
        ``,
        ...
        `---END REVIEW---`,
        ``,
        `Keep the source whole: do NOT split a complete article or markdown file into one wiki`,
        `page per heading or section. Emit exactly one wiki/sources/<slug>.md page that preserves`,
        `the document. Extra concept/entity pages only if they already exist in the index and this`,
        `source updates them — never a table-of-contents explosion.`,
        ``,
