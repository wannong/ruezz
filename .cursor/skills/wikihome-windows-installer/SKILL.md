...
description: >-
  Builds Ruezz's self-contained Windows NSIS installer (setup.exe) for
  ...
  打包, 打安装包, 正式安装包, NSIS, setup.exe, 给小白打包, ship a Windows installer,
   or release a Ruezz NSIS installer for a blank PC.
---

# Ruezz Windows installer
...
Deliverable: `release/Ruezz_<version>_x64-setup.exe` (typically ~150MB). Do not commit `release/`, `.cache/`, or staged runtimes.
...
- Stop Ruezz **dev** processes first (`pnpm --filter @wikihome/desktop dev`, vite, sidecar http). They lock `node_modules` binaries and make `pnpm install` / deploy fail with EPERM.
...
Remove leftover older `Ruezz_*_x64-setup.exe` in `release/` so 小白 only sees the current version.

### 5. Hand off

Give the user **one file**: `release/Ruezz_<ver>_x64-setup.exe`.
...
- Do **not** pack WebView2 Fixed Runtime by default (`webviewInstallMode: skip`). Prefer the OS WebView2 (look for `msedgewebview2.exe`, not a leftover registry `pv`). If missing, the app downloads a user-local runtime into `%LOCALAPPDATA%\Ruezz\webview2-runtime` (no admin).
- Sidecar spawn looks for `runtime/node.exe` + `sidecar/dist/cli.js` under resource dir, exe dir, or a short walk (skips `webview2-runtime` / `node_modules`). Sets `WIKIHOME_PYTHON`, `PYTHONNOUSERSITE`, `NODE_USE_ENV_PROXY`, `NODE_PATH`. Strips `\\?\` prefixes. Stderr goes to `%APPDATA%\Ruezz\sidecar-stderr.log`.
- Prefer OS WebView2; if missing, downloads into `%LOCALAPPDATA%\Ruezz\webview2-runtime` (no admin).
...
- `plugins.updater.endpoints`: GitHub `wannong/ruezz` `latest.json`
- `plugins.updater.pubkey`: contents of `.cache/signing/ruezz.key.pub` (public only)
