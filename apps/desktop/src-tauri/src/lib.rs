...
fn explain_io(what: &str, err: &std::io::Error) -> String {
    ...
    if broken {
        return format!(
            "{what}：引擎进程已退出。请关掉 Ruezz 再打开。若仍失败，查看 %APPDATA%\\Ruezz\\sidecar-stderr.log"
        );
    }
    if err.kind() == std::io::ErrorKind::NotFound {
        return format!("{what}：找不到引擎文件，请重新安装 Ruezz");
    }
    format!("{what}：{raw}")
}
...
fn spawn_sidecar(app: &AppHandle) -> Result<SidecarProc, String> {
    ...
    if let Ok(Some(status)) = child.try_wait() {
        ...
        return Err(if detail.is_empty() {
            format!(
                "引擎启动后立即退出（{status}）。请查看 %APPDATA%\\Ruezz\\sidecar-stderr.log"
            )
        } else {
...
mod win_msg {
    ...
    pub fn info(text: &str) {
        ...
        let caption: Vec<u16> = "Ruezz".encode_utf16().chain(std::iter::once(0)).collect();
        unsafe {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while running Ruezz");
}
